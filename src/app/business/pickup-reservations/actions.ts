...
export async function paymentCompleteAction(applicationId: number, businessId?: number): Promise<ActionResult> {
...
export async function waitingPickupAction(applicationId: number, businessId?: number): Promise<ActionResult> {
  const token = await getAuthToken();
  if (!token) return { success: false, error: "인증이 필요합니다." };
...
export async function receiveCompleteAction(applicationId: number, businessId?: number): Promise<ActionResult> {
  const token = await getAuthToken();
  if (!token) return { success: false, error: "인증이 필요합니다." };
...
export async function bulkWaitingPickupAction(
  ...
  try {
    await postApiUsersBusinessesPickupReservationsApplicationsWaitingPickup(request, { businessId }, withToken(token));
    revalidatePath("/business/pickup-reservations");
    return { success: true };
  } catch (error) {
