...
function PostTitle({ post }: { post: PostSummaryResponse }) {
  return (
    <span className="typo-medium-14 flex min-w-0 items-center gap-1 text-white">
...
const PostItem = memo(function PostItem({ post, isMobile, boardId }: PostItemProps) {
  ...
  if (isMobile) {
    return (
      ...
      >
        <span className="typo-medium-12 row-span-2 truncate border-r border-white/10 pr-2 text-center text-gray-400">
          {postTypeName}
        </span>
        <PostTitle post={post} />
        <div className="typo-medium-12 mt-1.5 flex min-w-0 items-center gap-2 text-gray-500">
  ...
  return (
    ...
    >
      <span className="typo-medium-12 truncate text-gray-400">{postTypeName}</span>
      <PostTitle post={post} />
      <span className="typo-medium-12 flex min-w-0 items-center justify-end gap-1.5 text-right text-gray-400">
        <span className="truncate">{authorNickname}</span>
        {post.authorAdmin ? <AdminAuthorBadge variant="icon" /> : null}
      </span>
      <span className="typo-medium-12 text-right text-gray-400">{post.viewCount ?? 0}</span>
      <span className="typo-medium-12 text-right text-gray-400">{formatDate(post.createdAt)}</span>
    </Link>
  );
});
