  ...
  return (
    <section aria-labelledby="general-item-sales-policy" className="mt-10 border border-white/10 bg-white/5 p-5 md:p-8">
      <div className="border-b border-white/10 pb-5">
        ...
        <p className="mt-2 text-sm leading-6 text-gray-400">주문 전에 배송과 교환·반품 조건을 확인해 주세요.</p>
      </div>

      <dl className="mt-6 grid gap-px border border-white/10 bg-white/10 sm:grid-cols-2">
        {deliveryPolicy.map((item) => (
          <div key={item.label} className="grid grid-cols-[110px_1fr] gap-3 bg-[#252c31] px-4 py-4 text-sm">
      ...
      <div className="mt-6 grid gap-3 text-sm leading-6 text-gray-300">
        <p>
      ...
      <Link
        href="/terms"
        className="mt-6 inline-flex text-sm font-semibold text-amber-500 underline-offset-4 transition-colors hover:text-amber-400 hover:underline"
      >
        전체 이용약관 및 판매정책 보기
      </Link>
    </section>
