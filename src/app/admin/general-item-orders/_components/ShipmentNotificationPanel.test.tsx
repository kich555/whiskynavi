...
import ShipmentNotificationPanel from "./ShipmentNotificationPanel";
const refresh = vi.fn();
vi.mock("next/navigation", () => ({ useRouter: () => ({ refresh }) }));
vi.mock("../shipment-notification-actions", () => ({ recoverShipmentNotification: vi.fn() }));
const recover = vi.mocked(recoverShipmentNotification);
const rows = [
...
describe("출고 알림 복구 화면", () => {
  ...
  it("사유 입력 후 선택한 채널의 재발송을 예약한다", async () => {
    ...
    await waitFor(() => expect(recover).toHaveBeenCalledWith(10, { notificationId: 1, reason: "고객 요청" }));
    await waitFor(() => expect(screen.getByRole("status")).toHaveTextContent("재발송을 예약"));
