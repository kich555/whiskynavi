...
interface SeriesEditContentProps {
  series: AdminBottleSeriesResponse;
  brandOptions: string[];
}

export default function SeriesEditContent({ series, brandOptions }: SeriesEditContentProps) {
  const { toggle } = useSidebar();
  const router = useRouter();
  const [isImageUploading, setIsImageUploading] = useState(false);
  const [brandValue, setBrandValue] = useState(series.brand ?? "");

  const [formState, formAction, isPending] = useActionState(updateSeriesAction.bind(null, series.id as number), {
    success: false,
  });

  return (
    <>
      <AdminHeader title="시리즈 편집" onToggleSidebar={toggle} showSearch={false} />

      <form action={formAction} className="p-8">
        <input type="hidden" name="representativeBottleId" value={series.representativeBottleId ?? ""} />
        ...
        <div className="flex gap-6 divide-x divide-gray-200 rounded-lg border border-gray-200 bg-white p-6">
          ...
          <div className="w-72 pl-6">
            ...
            <SingleImageUploader
              ...
              initialKey={series.imageKey ?? undefined}
              initialUrl={series.imageUrl ?? undefined}
