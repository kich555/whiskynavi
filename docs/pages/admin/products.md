...
| ID | 제품명 | 브랜드 | 증류소 | 시리즈 | 캐스크타입 | 도수 | 용량 | 병입일 | 관리 |
|----|--------|--------|--------|--------|------------|------|------|--------|------|
| 42 | 글렌피딕 18 | 글렌피딕 | 글렌피딕 | 쉐리 오크 | 쉐리 | 40% | 700ml | 2018.05.01 | [상세] |
...
- **정렬**: ID·병입일 헤더 클릭 → `FilterHeader` 드롭다운(오름/내림차순) — 두 헤더 모두 같은 `sort` 파라미터 키를 공유하므로 한 번에 하나의 정렬 기준만 적용됨(둘 다 선택 시 나중 선택이 우선)
