...
일반상품 주문 목록의 **비회원 안내 실패·발송 현황**에서 실패·재시도 중인 이메일/문자를 확인합니다. 전체 기록으로 전환하거나 이전 50건씩 볼 수 있고, 주문 링크로 상세에 진입합니다.

비회원 일반 품목의 즉시 배송 주문 상세에 채널별 상태, 마스킹된 수신처, 시도 횟수, 다음 재시도 시각, 관리자 처리 이력이 표시됩니다. **발송 접수**는 고객의 수신 완료를 뜻하지 않습니다. 자동 재시도는 최초 포함 최대 5회이며 같은 코드를 유지합니다.

- **재발송:** 성공 접수 또는 최종 실패한 채널을 선택하고 사유를 입력합니다. 대기 중인 건과 24시간 보관 기한이 지난 건은 선택할 수 없습니다.
- **조회 코드 재발급:** 주문자 본인 확인 완료 체크와 사유가 필수입니다. 기존 코드를 무효화하고 등록된 주문 연락처로만 새 코드를 안내합니다. 새 코드를 화면에 표시하지 않습니다.
- 처리 중 중복 클릭/모달 닫기를 막고, 실패하면 입력을 유지한 채 오류를 표시합니다. 인증 만료/권한 부족도 서버 액션과 백엔드에서 검사합니다.
- 기록은 새로고침으로 갱신하며, 표는 모바일에서 내부 가로 스크롤을 사용합니다. 보틀 주문과 회원 주문에는 이 복구 영역을 표시하지 않습니다.

고객 직접 코드 찾기, 연락처 변경, OTP 인증은 이번 화면 범위에 포함하지 않습니다.

### 이번 변경 검증 기록

- 전체 프론트 테스트 652개 및 프로덕션 빌드 통과. 변경 파일 ESLint 통과.
- 테스트 데이터로 데스크톱/390px 모바일, 표 내부 가로 스크롤, 본인 확인 필수 체크, 모달 완료 상태를 브라우저에서 확인했습니다. 실제 고객에게 메시지를 발송하지 않았습니다.
- 전체 `pnpm lint`는 기존 `repos/effect`의 오류로 실패했습니다. 별도 `tsc --noEmit`은 기존 `ManualPurchaseOrdersSection.test.tsx`, `business-pickup-location/actions.test.ts`, `admin/users/[userId]/page.test.tsx` 타입 오류 3건이 남습니다.
- `graphify` 실행 파일이 없어 그래프 갱신은 수행하지 못했고 페이지 문서는 수동 갱신했습니다.
...
일반상품 메뉴의 **출고 알림 관리**(`/admin/general-item-orders/shipment-notifications`)에서 이메일·문자 실패와 재시도 현황을 확인합니다. 기본은 실패 목록이며 전체 기록, ID 기준 이전 50건 탐색을 제공합니다. 주문 번호를 선택하면 해당 주문의 채널별 상태와 관리자 처리 이력을 확인하고 재발송할 수 있습니다. 일반상품 배송 주문 상세에도 바로가기 링크가 있습니다.

- 출고와 함께 저장한 발송 작업을 채널별 최대 5회 처리합니다. 송장을 변경하면 이전 송장의 대기 알림은 보내지 않습니다.
- 재발송은 최초 작업 생성 후 7일, 요청 간격 1분, 접수 완료 또는 최종 실패 상태에서 가능합니다. 처리 사유가 필수이며 대기 중 채널은 비활성화합니다. 서버가 현재 송장·주문 상태와 관리자 권한을 다시 검사합니다.
- 수신처는 마스킹하고 현재 등록된 연락처로만 발송합니다. 외부 발송 접수와 고객 수신 완료는 구분합니다. 성공 직후 장애 등으로 중복 안내될 가능성이 있어 재발송 전에 이력을 확인합니다.
- 서버 액션에서 인증 만료·관리자 권한·주문/작업 ID·사유를 검증합니다. 실패 시 모달과 입력을 유지하고 오류를 표시합니다. 처리 중에는 중복 요청과 모달 닫기를 막습니다.
- 모바일에서는 표 내부만 가로 스크롤되며, 재발송 모달은 화면 높이 안에서 스크롤됩니다. 독립적인 목록/이력 조회는 병렬로 처리합니다.
- 일반 품목 공고 수량 수정 API 타입에는 `expectedAvailableQuantity`와 `stockAdjustmentReason`을 추가했습니다. 공고 생성 화면의 동작은 동일합니다.

검증용 임시 화면에서 데스크톱과 390px 모바일, 사유 필수·대기 채널 비활성화·모달을 확인했습니다. 실제 발송은 하지 않았으며 임시 화면과 서버는 제거했습니다. `graphify`가 없어 페이지 문서는 수동 갱신했습니다.
