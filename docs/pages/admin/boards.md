...
| `/admin/board-management-history` | 게시글 삭제 감사 기록 및 작성자별 이력 | 특수 — 별도 설명 |
| `/admin/board-management-history/[postId]` | 관리기록 게시글 상세 | 관리자만 활성·삭제 글의 보존 본문 조회 |
...
### /admin/board-management-history — 게시글 관리기록 ⚠️ 특수

**목적**: 게시글에서 작성자의 활동과 관리자의 삭제 이력을 이어서 확인한다. 기본 탭은 관리자 삭제 기록이며, 작성 이력 탭은 활성 글과 삭제 글을 모두 포함한다. 글의 수정 버전 이력이나 과거 닉네임 스냅샷을 의미하지 않는다.

**삭제 기록 테이블**:

| 삭제 일시 | 게시글 | 작성자 | 처리 상태 / 삭제자 | 삭제 사유 |
|-----------|--------|--------|-------------------|-----------|
| 2026.09.17 14:30 | 중복 판매 글 (자유게시판 · #1024) | 몰트산책 (#58) | 관리자 삭제 / 내비운영자 (#3) | 동일 내용 반복 게시 |

- **닉네임 표기**: 현재 닉네임을 우선 표시하고 ID를 보조로 표기. 계정 정보가 없으면 “닉네임 확인 불가” 표시. 닉네임 변경 후에도 ID 기준으로 동일인의 이력 추적
- **추적 링크**: 작성자 닉네임 → `mode=posts&authorId=...` (해당 작성자의 활성·삭제 게시글 전체), 삭제 관리자 닉네임 → `mode=deleted&deletedBy=...` (해당 관리자의 삭제 이력)
- **검색**: 게시글 제목, 작성자 닉네임, 삭제 관리자 닉네임의 부분 검색. 특정 사람의 이력으로 들어오면 한정 조건을 표시하며 개별 해제 가능. 검색 시 1페이지로 이동
- **작성 이력의 삭제 상태 필터**: 전체(기본) / 게시 중 / 삭제 전체 / 작성자 삭제 / 관리자 삭제 / 삭제 주체 미상. 선택 후 검색을 누르면 제목·작성자 조건과 함께 서버 조회에 적용되며, 총 건수와 페이지도 필터 기준으로 계산한다. 삭제 전체는 삭제 주체와 무관한 모든 삭제 글을 포함한다
- **필터 유지·해제**: 상태는 페이지 이동과 상세 복귀 시 유지. 초기화하면 전체로 복귀하고, 관리자 삭제 기록 탭으로 전환하면 상태 필터를 해제한다. 관리자 삭제 기록 탭에는 상태 선택창을 표시하지 않는다
- **상태 구분**: 게시 중 / 관리자 삭제 / 작성자 삭제 / 삭제 주체 미상. 작성자 직접 삭제를 관리자 삭제로 취급하지 않음
- **정렬과 페이지**: 삭제 기록은 삭제 최신순, 작성 이력은 작성 최신순. 기본 20건, 10/20/50/100건 선택. 조건과 페이지는 URL에 유지하며 범위를 벗어난 페이지는 마지막 유효 페이지로 이동
- **반응형**: 데스크톱(1440px 이상)은 테이블, 작은 화면은 카드. 긴 제목과 사유는 줄바꿈하며 전체 사유는 상세에서 확인
- **상세 조회**: 글 제목 → `/admin/board-management-history/[postId]`. 보존 본문, 작성자, 작성·최종 수정 일시, 삭제자·삭제 사유 표시. 본문 HTML은 서버에서 정화하며 조회수는 증가하지 않음
- **상세 탐색**: 조회하던 검색 조건·페이지로 돌아가기, 작성자의 전체 게시글/관리자 삭제 기록, 관리자의 전체 삭제 기록, 작성자 회원 정보 연결
- **권한과 오류**: 관리자 전용 읽기 API 사용. 404는 게시글 없음 안내, API 오류는 빈 기록으로 숨기지 않고 오류 화면 표시. 시각은 한국 표준시로 고정

**도메인 연결**: 기존 게시글 삭제가 보존한 제목·본문·삭제 주체·삭제 사유를 조회한다. 2.0 관리기록 API를 사용하므로 백엔드를 먼저 배포한 뒤 프론트를 배포한다.

상태 필터도 `status` 파라미터를 지원하는 백엔드를 먼저 배포해야 한다. DB 변경은 없으며, 이전 백엔드는 파라미터를 무시할 수 있어 프론트 단독 선배포는 피한다.
