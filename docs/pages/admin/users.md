...
| `/admin/user-purchase-statistics` | 회원 구매 통계            | 올해 예약 확정 구매의 일일 집계·검색·필터·정렬 |
...
### /admin/user-purchase-statistics — 회원 구매 통계

현재 연도 예약 구매를 회원별·채널별로 보여주는 관리자 전용 목록. 회원 메뉴에서 회원 관리 바로 아래에 위치한다.

| ID  | 이름 | 사용자명 | 내비 회원 여부 | 내비 구매 병수 | 내비 구매 종류 수 | 테일즈 회원 여부 | 테일즈 구매 병수 | 테일즈 구매 종류 수 |
| --- | ---- | -------- | -------------- | -------------- | ----------------- | ---------------- | ---------------- | ------------------- |

- **집계 기준**: 예약 판매 보틀 중 예약 확정 이후의 유효 주문 상태만 포함. 취소 요청·취소 완료 주문과 `ADMIN_MANUAL` 결제 방식은 제외하며, 내비/테일즈는 보틀 `brand`로 구분
- **갱신 방식**: 매일 오전 3시(Asia/Seoul)에 현재 연도 전체 범위를 다시 계산하며, 관리자는 화면의 `지금 갱신` 버튼으로 같은 비동기 집계를 요청할 수 있음
- **검색**: ID·이름·사용자명
- **목록 대상**: 사업자 역할(`ROLE_BUSINESS`, `ROLE_TRAILNTALE_BUSINESS`, `ROLE_COMMUNITY_BUSINESS`, `ROLE_PICK_UP_BUSINESS`)을 가진 회원은 제외
- **필터**: 현재 내비/테일즈 회원 여부, 해당 연도 내비/테일즈 최소 구매 병수
- **등급 부여**: 채널별 비회원 배지 옆 `등급 부여` 버튼으로 내비 커뮤니티 또는 테일즈 커뮤니티 회원 역할을 즉시 추가. 성공 후 목록을 다시 조회하며 제거는 멤버십 관리에서 수행
- **정렬**: 표의 9개 열 모두 오름차순·내림차순 지원
- **페이지네이션**: URL의 `page`/`limit` 기반, 페이지당 최대 100명
...
- **역할 체계**: 백엔드 ROLE\_\* 역할 — 커뮤니티 멤버십·비즈니스 사업자 구분의 기준
