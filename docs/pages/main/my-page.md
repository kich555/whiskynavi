...
| 수령 완료 확인 모달 | 수령 대기 주문의 최종 수령 확인 | 되돌릴 수 없는 상태 변경임을 경고하고 재확인. 성공 시 목록 재검증과 현재 화면 새로고침 |
...
- **수령 완료 처리**: 픽업 주문이 `ORDER_PREPARING`, `PAYMENT_COMPLETED`, `RECEIPT_PENDING` 중 하나일 때 카드와 상세 화면에서 버튼 노출. 클릭 시 별도 경고창에서 되돌릴 수 없음을 확인한 뒤 최종 확정
...
2. 주문 상태 배지 + 주문일. 취소 가능 상태면 "주문 취소" 버튼, 픽업 주문이 준비 중/결제 완료/수령 대기 상태면 "수령완료 처리" 버튼
...
수령 완료는 인증 사용자가 본인 소유 픽업 주문에만 실행할 수 있고, 서버가 현재 상태가 `ORDER_PREPARING`, `PAYMENT_COMPLETED`, `RECEIPT_PENDING` 중 하나인지 다시 검증한다. 확정 후에는 이전 상태로 되돌리는 사용자 동작을 제공하지 않는다.
...
- 변경 액션(주문 취소/수령 완료, 프로필/비밀번호 변경, 사업자 신청/취소, 문의 생성/메시지 추가/삭제)은 Server Action → Orval API. `revalidatePath('/my-page')` 또는 `/my-page/inquiries`로 캐시 갱신.
