...
**RSC 데이터 페칭**: 사용자 정보, 주문 목록, 사업자 신청 개요를 `Promise.all` 병렬 페칭. 주문 목록은 인증 사용자의 주문만 반환하는 `GET /api/2.0/orders`를 사용하며 페이지 크기는 10건이다. 각 API는 독립적으로 catch하고, 주문 API 실패는 빈 결과와 구분되는 오류/재시도 UI로 표시한다.
...
- **상태 필터**: 전체 또는 사용자용 주문 상태 10종을 선택. 상태를 선택하면 관리자 수동입력 주문은 제외된다.
- **수동입력 내역조회**: 주문 상태 필터 왼쪽에 작은 스위치로 배치하며, `manualOnly=true`로 관리자 수동입력 주문만 조회. 활성화하면 상태 필터를 해제하고, 상태 필터를 선택하면 수동입력 내역조회를 해제한다.
- **정렬**: 주문 생성일 최신순(`CREATED_AT`, 기본) / 병입일 최신순(`BOTTLED_DATE`). 필터나 정렬 변경 시 첫 페이지로 이동한다.
- **URL 상태**: `orderStatus`, `manualOnly`, `sort`, `page`, `tab`을 URL에 보존하므로 페이지 이동과 새로고침 뒤에도 같은 조회 조건을 유지한다.
...
- **수동입력 상태 비노출**: `orderSource=ADMIN_MANUAL` 주문은 응답에 `orderStatus`가 없다. 목록 상태 배지, 상세 상태 영역, 배송 진행 단계, 상태 기반 액션과 대체 상태 문구를 모두 렌더링하지 않는다.
- **페이지네이션**: URL 기반(`?page=&tab=orders`) — 현재 필터와 정렬 조건을 유지하며 북마크/공유 가능
- **조회 상태**: 라우트 로딩 스켈레톤과 필터 이동 중 로딩 문구를 제공하고, 빈 결과와 API 오류를 구분한다.
...
- 세금계산서 수신 이메일은 선택 항목이며 회원 이메일을 기본값으로 채운다. 다른 주소로 변경하거나 비워서 제출할 수 있고, 입력한 경우 이메일 형식과 최대 254자를 검증한다.
- 파일 선택 및 제출 직전에 10MB 제한을 검사한다. 초과 파일은 전송하지 않고 폼에 안내하며, 정상 파일로 교체하면 다시 제출할 수 있다.
- 신청은 브라우저에서 인증 토큰과 함께 Navi API로 직접 전송한다. 첨부파일이 Vercel의 요청 용량 제한에 걸리지 않도록 Server Action을 경유하지 않는다.
- 제출 실패 시 입력 내용과 첨부파일을 유지하며, 성공 시 마이페이지 갱신 후 오버레이를 닫는다.
...
2. 일반 주문은 주문 상태 배지 + 주문일을 표시. 취소 가능 상태면 "주문 취소" 버튼, 픽업 주문이 준비 중/결제 완료/수령 대기 상태면 "수령완료 처리" 버튼. 수동입력 주문은 상태 영역과 상태 기반 버튼을 표시하지 않음
...
6. 배송 정보(배송 데이터 있을 때만): 배송 진행, 수령인, 연락처, 주소, 배송 메모, 배송사(기본 CJ대한통운), 운송장번호, 발송일, 배송완료일. 수동입력 주문은 배송 진행 항목을 생략
...
- 메인은 `Promise.all`로 사용자/2.0 주문내역/사업자개요를 병렬 페칭하고, 개별 catch로 부분 실패를 허용.
...
- 탭 전환과 주문 필터/정렬/페이지네이션은 URL searchParams 기반 — 북마크/공유 가능. 탭은 `replaceState`로 URL만 갱신(재페칭 없음), 주문 조회 조건 변경은 App Router 탐색으로 RSC 데이터를 다시 가져온다.
